...
class CameraServiceModel: ObservableObject {
...
// MARK: Session Management Properties
    ...
    func configure() {
        model.scannedKey = metadataProcessor.lastValidKeyObject
        sessionQueue.async {
    ...
    default:
        ...
        DispatchQueue.main.async {
            ...
            self.model.shouldShowAlertView = true
            self.model.isCameraUnavailable = true
            self.model.isCameraButtonDisabled = true
        }
    }
}
...
//                MARK: Here enable capture button due to successfull setup
                self.model.isCameraButtonDisabled = false
            }
        }
    }
...
//        We use our capture session queue to ensure our UI runs smoothly on the main thread.
        sessionQueue.async {
            ...
            case .success:
                ...
                DispatchQueue.main.async {
                    self.model.isCameraButtonDisabled = false
                    self.model.isCameraUnavailable = false
                }
    ...
    func capturePhoto() {
        ...
        sessionQueue.async {
            ...
            let photoCaptureProcessor = PhotoCaptureProcessor(with: photoSettings, willCapturePhotoAnimation: { [weak self] in
                ...
                DispatchQueue.main.async {
                    self?.model.willCapturePhoto = true
                }
                
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) {
                    self?.model.willCapturePhoto = false
                }
                ...
                if animate {
                    self?.model.shouldShowSpinner = true
                } else {
...
//        }
    ...
    private func initialSessionConfiguration() {
        ...
        model.$cameraMode.dropFirst().receive(on: sessionQueue).sink { [weak self] newMode in
    ...
    private func startCapturingVideo() {
        ...
        sessionQueue.async {
            ...
            self.toggleTorch(on: self.model.flashMode == .on)
